...
<tr>
	...
	<tr>
		...
		<td width="" height="28" colspan="3" align="center" background="../images/bill/icon_05.gif" class="fontStyle">通话详单</td>
		<td>
	...
	<tr>
		...
		<table width="100%">
          ...
          <tr>
            ...
            <td width="79%" class="titleblue">通话详单</td>
            <td width="5%">&nbsp;</td>
          </tr>
        </table></td>
          </tr>
	...
	<tr>
		...
		<td width="397" height="59" background="../images/main_14.jpg"><table width="95%">
          <tr>
            <td align="right"><input name="button" type="submit" class="input_button02" id="button" value="返回" onClick="window.location.href='bill.action?op=tel&user_tel=<ww:property value="user_tel"/>';"></td>
          </tr>
        </table></td>
		<td>
...
<script type="text/javascript">
	...
	function startRequest(){ 
		...
		xmlHttp.open("POST", "domain_action.jsp?Bcode=100001000300003&AccCode=<ww:property value="user_tel"/>&Month=<ww:property value="month"/>&Password=", true); 
	...
	function outputList(Itemroot){ 
		...
		innerStr +="<tr>";
		innerStr +="<td align=\"center\" bgcolor=\"#179CBD\" class=\"STYLE13\">起始时间</td>";
		innerStr +="<td align=\"center\" bgcolor=\"#179CBD\" class=\"STYLE13\">通话时长（秒）</td>";
		...
		//innerStr +="<td align=\"center\" bgcolor=\"#179CBD\" class=\"STYLE13\">对方所在地</td>";
		innerStr +="<td align=\"center\" bgcolor=\"#179CBD\" class=\"STYLE13\">长途类型</td>";
		//innerStr +="<td align=\"center\" bgcolor=\"#179CBD\" class=\"STYLE13\">基本或漫游费</td>";
		//innerStr +="<td align=\"center\" bgcolor=\"#179CBD\" class=\"STYLE13\">长途费</td>";
		innerStr +="<td align=\"center\" bgcolor=\"#179CBD\" class=\"STYLE13\">小计</td>";
		innerStr +="</tr>";
			for(var i=begin;i <end;i++){ 
				CallTime = Itemroot[i].getElementsByTagName("CallTime");
				CallTimeLong = Itemroot[i].getElementsByTagName("CallTimeLong");
				...
				LongType = Itemroot[i].getElementsByTagName("LongType");
				//Cost1 = Itemroot[i].getElementsByTagName("Cost1");
				//Cost2 = Itemroot[i].getElementsByTagName("Cost2");
				Costs = Itemroot[i].getElementsByTagName("Costs");
				if(CallTime[0].childNodes[0] !=null)
				{
					useCallTime = CallTime[0].childNodes[0].nodeValue.substring(4,15);
				}
				if(CallTimeLong[0].childNodes[0] !=null)
				{
					useCallTimeLong = CallTimeLong[0].childNodes[0].nodeValue;
				}
				if(CallType[0].childNodes[0] !=null)
				{
					if(CallType[0].childNodes[0].nodeValue == 1)
					useCallType = "主叫";
					if(CallType[0].childNodes[0].nodeValue == 2)
					useCallType = "被叫";
				}
				...
				if(LongType[0].childNodes[0] !=null)
				{
					useLongType = LongType[0].childNodes[0].nodeValue.substring(0,2);
					if(useLongType =="国内")
						useLongType="长途";
				}
				//if(Cost1[0].childNodes[0] !=null)
				//{
				//	useCost1 = Cost1[0].childNodes[0].nodeValue;
				//}
				//if(Cost2[0].childNodes[0] !=null)
				//{
				//	useCost2 = Cost2[0].childNodes[0].nodeValue;
				//}
				if(Costs[0].childNodes[0] !=null)
				{
					useCosts = Costs[0].childNodes[0].nodeValue;
					if(useCosts>0)
						useCosts = useCosts/1000;
				}	
				innerStr +="       <tr>";
				innerStr +="             <td align=\"center\" bgcolor=\"#F3F1F0\" class=\"STYLE14\">"+useCallTime+"</td>";
				innerStr +="             <td align=\"center\" bgcolor=\"#F3F1F0\" class=\"STYLE14\">"+useCallTimeLong+"</td>";
				...
				innerStr +="             <td align=\"center\" bgcolor=\"#F3F1F0\" class=\"STYLE14\">"+useLongType+"</td>";
				//innerStr +="             <td align=\"center\" bgcolor=\"#F3F1F0\" class=\"STYLE14\">"+useCost1+"</td>";
				//innerStr +="             <td align=\"center\" bgcolor=\"#F3F1F0\" class=\"STYLE14\">"+useCost2+"</td>";
  		...
  		innerStr +="  <td class=\"font12 STYLE4\" colspan=2 align=right>当前第"+pagenum+"页&nbsp;共"+realpage+"页";
   		innerStr +="  <a id=\"down\" href=\"bill.action?op=tel_detail&pageno=1&month=<ww:property value="month"/>&user_tel=<ww:property value="user_tel"/>\">";
   		innerStr +="  首  页</a>";
   		innerStr +="  <a id=\"down\" href=\"bill.action?op=tel_detail&pageno="+pre+"&month=<ww:property value="month"/>&user_tel=<ww:property value="user_tel"/>\">";
   		innerStr +="  上一页</a>";
   		innerStr +="  <a id=\"down\" href=\"bill.action?op=tel_detail&pageno="+next+"&month=<ww:property value="month"/>&user_tel=<ww:property value="user_tel"/>\">";
   		innerStr +="  下一页</a>";
   		innerStr +="  <a id=\"tail\" href=\"bill.action?op=tel_detail&pageno="+realpage+"&month=<ww:property value="month"/>&user_tel=<ww:property value="user_tel"/>\">";
   		innerStr +="  最后一页</a></td>";
   		innerStr +="  </tr>";
   		innerStr +="  </table></td>";
		innerStr +="  </tr>	";
